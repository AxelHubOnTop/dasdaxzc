-- ANIME RANGER X SCRIPT MADE BY Chalarm
...
local function GetConfigPath()
    return "AxelHub/AnimeRangerX[Premium]/" .. player.Name .. ".json"
end
...
local function autoStat(inst, target, statsFilter, delay, statusLb)
    target = target or "O+"
    if not inst or not inst.Parent then if statusLb then statusLb.Text = "❌ Unit not found" end; return false end
    local tag = getTag(inst)
    if not tag then if statusLb then statusLb.Text = "❌ Tag not found" end; return false end
    ...
    if total == 0 then if statusLb then statusLb.Text = "✅ Stats already "..target.."!" end; return true end
    if getBoosters() < total then if statusLb then statusLb.Text = ("❌ Missing Boosters! (%d/%d)"):format(getBoosters(), total) end; return false end
    ...
    local function onTraitChange()
        ...
        if idx then
            ...
            sendWebhook({ embeds = {{
                ...
                image={url=GIF}, footer={text="AxelHub • Premium Script", icon_url=LOGO}, timestamp=os.date("!%Y-%m-%dT%H:%M:%SZ"),
            }}})
...
function Curse:reset()
    self.resetting = true
    if self.toggleRef then self.toggleRef.Value = false end
...
function Curse:start(toggle, label)
    ...
    self.running = true
    local count, startF = 0, getFingers()
    task_spawn(function()
...
local function sendTestWebhook()
    ...
    sendWebhook({ embeds = {{
        ...
        footer = {text="AxelHub • Premium Script", icon_url=LOGO},
        timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ"),
    }}}, Cfg.webhookUrl)
end
...
do
    ...
    local function runAutoJoin()
        ...
        if isInGame() then
            ...
            if mode:find("Ranger Stage") or mode:find("Challenge") then
                if ajStatus then ajStatus.Text = "⚠️ Current Mode: " .. mode .. " (Waiting for end...)" end
        ...
        if ajStatus then ajStatus.Text = "⚙️ Configuring Room..." end
        ...
        if ajStatus then ajStatus.Text = "📋 Submitting Room..." end
        fireRoom("Submit"); task.wait(0.4)

        runCountdown(AJ.startDelay, function()
            if not AJ.running then return end

            if ajStatus then ajStatus.Text = "▶️ Starting Match..." end
            fireRoom("Start")
            if ajStatus then ajStatus.Text = "🎮 Waiting to Enter..." end
            ...
            task.spawn(function()
                ...
                while waited < 45 and AJ.running do
                    ...
                    if ajStatus then ajStatus.Text = ("🎮 Waiting... [%s] %ds"):format(bar, left) end
                end
    ...
    local tab  = lobbySection:Tab({ Selected=true, Title="Auto Join", Icon="rbxassetid://10723405649" })
    local form = tab:Form()
    ...
    local wrRow = form:Row({SearchIndex="Choose World"})
    wrRow:Left():TitleStack({ Title="🌍 Choose World", Subtitle="Select world to farm" })
    ...
    local chRow = form:Row({SearchIndex="Choose Chapter"})
    chRow:Left():TitleStack({ Title="📖 Chapter", Subtitle="Select chapter to farm" })
    ...
    tr:Left():TitleStack({ Title="🚀 Auto Join", Subtitle="Enable Story Auto Farm" })
    ...
    local function runRangerStage()
        ...
        if isInGame() then
            ...
            if mode:find("Story") or mode:find("Challenge") then
                if rsStatus then rsStatus.Text = "⚠️ Current Mode: " .. mode .. " (Waiting for end...)" end
            ...
            if rsStatus then rsStatus.Text = "🎮 In Match [" .. mode .. "] Waiting for Results" end
            waitForGameEnd(300, Cfg.rs_endAction or "won_next_def_retry")
            if RS.running then task.spawn(runRangerStage) end
            return
        end
        ...
        if rsStatus then rsStatus.Text = "⚙️ Configuring Room..." end
        ...
        if rsStatus then rsStatus.Text = "📋 Submitting Room..." end
        fireRoom("Submit"); task.wait(0.4)
        runCountdown(RS.startDelay, function()
            if not RS.running then return end
            if rsStatus then rsStatus.Text = "▶️ Starting Match..." end
            fireRoom("Start")
            if rsStatus then rsStatus.Text = "🎮 Waiting to Enter..." end
            task.spawn(function()
                local waited = 0
                while waited < 45 and RS.running do
                    ...
                    if rsStatus then rsStatus.Text = ("🎮 Waiting... [%s] %ds"):format(bar,left) end
                end
                if not isInGame() then
                    RS.running = false
    ...
    rsChRow:Left():TitleStack({ Title="🎯 Stage", Subtitle="Select Ranger Stage" })
    ...
    rsFr:Left():TitleStack({ Title="🔒 Friend Only (RS)", Subtitle="Only friends can join" })
    ...
    local rsDt = rsDr:Left():TitleStack({ Title="⏱️ Delay RS ("..tostring(Cfg.rs_startDelay or 3).."s)", Subtitle="1-10 seconds" })
    ...
    rsTr:Left():TitleStack({ Title="🚀 Ranger Stage Auto Join", Subtitle="Enable Ranger Stage Auto Farm" })
...
do
    ...
    for i=1, 6 do
        local sr = form:Row()
        local slTitle = sr:Left():TitleStack({ Title = "   ↑ Max Lvl [Slot "..i.."]", Subtitle = "Upgrade steps" })
        sr:Right():Slider({
    ...
    for pri = 1, 6 do
        ...
        priRow:Left():TitleStack({ Title="🥇 Priority "..pri, Subtitle="Upgrade this first as rank "..pri })
    ...
    rowVotePlay:Left():TitleStack({ Title="▶️ Auto Vote Play", Subtitle="Vote play again (Once per match)" })
...
do
    ...
    local ir3 = form:Row()
    ir3:Left():TitleStack({ Title="Claim Daily Login 1-7" })
    ir3:Right():Button({ Label="🎁 Claim Daily 1-7", State="Accent", Pushed=function()
...
do
    ...
    local cr3 = form:Row()
    cr3:Left():TitleStack({ Title="🚀 Auto Join Challenge", Subtitle="Create and auto join challenges" })
    ...
    jwiRow:Left():TitleStack({ Title="🎁 Join When Have Item", Subtitle="Join challenge only if it has rewards below" })
    ...
    scRow:Left():TitleStack({ Title="🧠 Smart Challenge", Subtitle="Priority: Challenge (Have Item) > Other Mode" })
...
local function setupTraitTab(title, icon, isMain)
    ...
    charRow:Left():TitleStack({ Title="Unit 🎃", Subtitle="Choose Unit For "..title })
    ...
    charLb = form:Row():Left():Label({ Text="Character not selected yet 🎃", TextWrapped=true })

    local tRow = form:Row()
    tRow:Left():TitleStack({ Title="Trait 📍", Subtitle="Target traits for "..title })
    tRow:Right():PullDownButton({ Options=ALL_TRAITS, Label=" ", Multi=true, ValueChanged=function(s, v)
        ...
        traitLb.Text = #wants==0 and "Trait not selected yet" or table_concat(wants, ", ")
    end })
    local tlRow = form:Row()
    traitLb = tlRow:Left():Label({ Text="Trait not selected yet", TextWrapped=true })
    tlRow:Right():Button({ Label="Clear", State="Minimal", Pushed=function() wants={}; traitLb.Text="Trait not selected yet" end })
    ...
    createTog("Reroll "..title, "Reroll", false)
    createTog("INF Reroll", "INF Reroll", true)
end

setupTraitTab("Main Trait", "rbxassetid://10747373176", true)
setupTraitTab("Sub Trait", "rbxassetid://10723424963", false)
...
do
    ...
    selRow:Left():TitleStack({ Title="💊 Choose Capsule", Subtitle="Choose Capsule You Want To Open" })
    ...
    modeRow:Left():TitleStack({ Title="🎲 Opening Mode", Subtitle="Choose How Many You Want To Open" })
    modeRow:Right():PullDownButton({
        Options = {"Open Amount", "Open All"},
        Label = Cfg.cap_openMode == "all" and "Open All" or "Open Amount",
        ValueChanged = function(self, v)
    ...
    local amtTitle = amtRow:Left():TitleStack({ Title=("Number To Open 🔢 (%d)"):format(Cfg.cap_customAmount or 1) })
    amtRow:Right():Slider({
        ...
        ValueChanged = function(_, v)
            ...
            amtTitle.Title = ("Number To Open 🔢 (%d)"):format(math.floor(v))
            saveConfig()
        end
    })

    local sRow = form:Row()
    sRow:Left():TitleStack({ Title="Auto Sell Character 💰", Subtitle="Sell Trash Characters" })
    sRow:Right():Toggle({ Value=Cfg.cap_autoSell, ValueChanged=function(_, v) Cfg.cap_autoSell=v; saveConfig() end })

    local ksRow = form:Row()
    ksRow:Left():TitleStack({ Title="Keep Shiny ✨", Subtitle="Don't Sell Shiny Characters" })
...
do
    ...
    --  RATEUP SECTION
    form:Row():Left():Label({ Text=" 🌟 RateUp Banner ", TextWrapped=true })
    ...
    --  STANDARD SECTION 
    form:Row():Left():Label({ Text=" 📖 Standard Banner ", TextWrapped=true })
    ...
    --  PITY & AUTO SELL
    local pityLb = form:Row():Left():Label({ Text="🔥 Pity: Loading...", TextWrapped=true })
    form:Row():Left():Label({ Text=" 💰 Auto Sell Unit ", TextWrapped=true })
    ...
    --  CURRENCY (Bottom) 
    form:Row():Left():Label({ Text=" 💎 Currency ", TextWrapped=true })
            ...
            if window.Minimized == false then
                pcall(function()
                    ...
                    if rup then
                        ...
                        for _, c in ipairs(rup) do
                            if c.name ~= "" then
                                ...
                                t = t .. " • [" .. c.rarity .. "] " .. c.name .. " (" .. c.rate .. ")" .. rst .. "\n"
                            end
                        end
                        local rtRes = playerGui.UnitsGacha.Main.TimeBar.RateUpReset.Numbers
                        t = t .. "⏳ RateUp Reset: " .. (rtRes.ContentText ~= "" and rtRes.ContentText or rtRes.Text)
                        trackerRateup.Text = centerText(t, 12)
                    end
                    ...
                    if std then
                        ...
                        t = t .. "⏳ Standard Reset: " .. (reset.ContentText ~= "" and reset.ContentText or reset.Text)
                        trackerStandard.Text = centerText(t, 12)
                        
                        -- Update Standalone Pity Label
                        if pityLb then
                            pityLb.Text = "🔥 Pity: " .. (pity.ContentText ~= "" and pity.ContentText or pity.Text)
                        end
                    end
                    ...
                    if gold then goldLb.Text = "💰 Gold: " .. (gold.ContentText ~= "" and gold.ContentText or gold.Text) end
                end)
            end
            task_wait(1)
        end
...
do
    ...
    do
        ...
        local er = fm:Row()
        er:Left():TitleStack({ Title="Send Webhook", Subtitle="Notify When Successful" })
        ...
        local pr = fm:Row()
        pr:Left():TitleStack({ Title="Enable Ping", Subtitle="Ping Discord When Successful" })
    ...
    do
        ...
        local items = {
            { k="darkMode",   title="Dark Mode",        sub="Dark Theme",          cb=function(v) app.Theme=v and cascade.Themes.Dark or cascade.Themes.Light end },
            { k="dropshadow", title="Window Shadow",       sub="Add UI Shadow",          cb=function(v) window.Dropshadow=v end },
        ...
        local kr = fm:Row()
        kr:Left():TitleStack({ Title="Keybind Toggle", Subtitle="Open/Close UI" })
    ...
    do
        ...
        local statusLb = fm:Row():Left():Label({ Text="💤 Off", TextWrapped=true })

        local togRow = fm:Row({SearchIndex="Tag Viewer"})
        togRow:Left():TitleStack({
            Title    = "🏷️ Show Tag in Game",
            Subtitle = "Append tag to unit name in Collection GUI | Only you can see"
        })
        togRow:Right():Toggle({ ValueChanged=function(self, v)
            if v then
                ...
                statusLb.Text = count > 0
                    and ("✅ tag %d units | Realtime"):format(count)
                    or  "Open the unit inventory to check the tag"
                task.spawn(function()
                    while tagRunning do
                        task.wait(1)
                        ...
                        local n = applyTags()
                        if n > 0 then
                            statusLb.Text = ("✅ tag %d units | Realtime"):format(n)
                        end
                    end
                    restoreTags()
                    statusLb.Text = "💤 Off"
