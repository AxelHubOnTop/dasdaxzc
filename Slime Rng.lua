...
_G.Key = "DEV-F8A3-Y036-0ITD-PRCB"
...
_G.DefaultSettings = {
        ...
        folder_name = "AxelHub/"..userid.."_SRNG.JSON"
...
userid = game.Players.LocalPlayer.UserId
    return "AxelHub/"..userid.."_SRNG.JSON"
end
...
toggles["Auto_Roll"] = Main:Toggle({
    Title = "Auto Roll",
    Desc = "",
    Icon = "circle-check-big",
    Type = "Checkbox",
    Value = _G.Settings.Auto_Roll or false, -- default value
    Callback = function(state) 
        _G.Settings.Auto_Roll = state
    end
})

toggles["Auto_Purchase_NewZone"] = Main:Toggle({
    Title = "Auto Purchase New Zone",
    Desc = "",
    Icon = "circle-check-big",
    Type = "Checkbox",
    Value = _G.Settings.Auto_Purchase_NewZone or false, -- default value
    Callback = function(state) 
        _G.Settings.Auto_Purchase_NewZone = state
    end
})

toggles["Auto_Equip_Best"] = Main:Toggle({
    Title = "Auto Equip Best",
    Desc = "",
    Icon = "circle-check-big",
    Type = "Checkbox",
    Value = _G.Settings.Auto_Equip_Best or false, -- default value
    Callback = function(state) 
        _G.Settings.Auto_Equip_Best = state
    end
})
...
if isfolder("AxelConfig_SRNG") then
for i,v in pairs(listfiles("AxelConfig_SRNG")) do
local name = v:match("[^/\\]+$")
table.insert(configall, name)
end
else
    makefolder("AxelConfig_SRNG")
end
...
local Button = Settings:Button({
    ...
    Locked = false,
    Callback = function()
if not isfile("AxelConfig_SRNG/"..Config_Name..".JSON") then
    writefile("AxelConfig_SRNG/"..Config_Name..".JSON", "")
end
    end
})
...
local Button = Settings:Button({
    ...
    Locked = false,
    Callback = function()
delfile("AxelConfig_SRNG/"..Config_Select)
    end
})
...
local Button = Settings:Button({
    ...
    Locked = false,
    Callback = function()
if not isfile("AxelConfig_SRNG/"..Config_Select) then
    writefile("AxelConfig_SRNG/"..Config_Select, game:service('HttpService'):JSONEncode(_G.DefaultSettings))
else

SaveConfig("AxelConfig_SRNG/"..Config_Select)
end
    end
})
...
prop = game:service('HttpService'):JSONDecode(readfile("AxelConfig_SRNG/"..Config_Select))
...
for i,v in pairs(workspace:GetChildren()) do
	if string.find(v.Name, "Gameplay") then
        gameplay = v
	end
end

function checkin()
for i,v in pairs(gameplay.Enemies:GetChildren()) do
	local dsisd = (v.RootPart.Position-game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
	if dsisd <= 15 then
	return true
	end
end
...
for i,v in pairs(workspace.Zones:GetChildren()) do
	if v.Gate.Back.CanCollide == false and tonumber(v.Name) > a then
a = tonumber(v.Name)
return tostring(tonumber(v.Name)+1)
	end
end
end
...
local distance = (workspace.Zones[nowzone()].POI.Hitbox.Position-game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude

if distance >= 60 then
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = workspace.Zones[nowzone()].POI.Hitbox.CFrame
return true
else
for i,v in pairs(gameplay.Enemies:GetChildren()) do
	if not checkin() then
...
while task.wait(2) do
	pcall(function()
	if _G.Settings.Auto_Purchase_NewZone then
-- This code was generated by Cobalt
-- https://github.com/notpoiu/cobalt

local Event = game:GetService("ReplicatedStorage").Packages._Index["leifstout_networker@0.3.1"].networker._remotes.ZonesService.RemoteFunction
Event:InvokeServer(
    "requestPurchaseZone"
)
	end
...
while task.wait(3) do
	pcall(function()
	if _G.Settings.Auto_Equip_Best then
-- This code was generated by Cobalt
-- https://github.com/notpoiu/cobalt

local Event = game:GetService("ReplicatedStorage").Packages._Index["leifstout_networker@0.3.1"].networker._remotes.InventoryService.RemoteFunction
